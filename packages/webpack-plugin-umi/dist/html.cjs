...
function _findBuild(js) {
  ...
  let buildEntry = '';
  js.forEach(j => {
    var _j$match, _j$match2;
    const [_0, core] = (_j$match = j.match(REG_CACHED_CORE)) !== null && _j$match !== void 0 ? _j$match : []; // eslint-disable-line no-unused-vars
    if (core) {
      buildCore = core;
    }
    const [_1, entry] = (_j$match2 = j.match(REG_CACHED_ENTRY)) !== null && _j$match2 !== void 0 ? _j$match2 : []; // eslint-disable-line no-unused-vars
    if (entry) {
      buildEntry = entry;
    }
  });
...
class WebpackPluginUmiHtml {
  constructor(options = {}) {
    this.options = options;
  }
  apply(compiler) {
    compiler.hooks.compilation.tap('UmiHtml', compilation => {
      HtmlWebpackPlugin.getHooks(compilation).beforeAssetTagGeneration.tapAsync('UmiHtml', (data, callback) => {
        ...
        const version = `${this.options.version}.${build}`;
        // const context = '/app-a';

        const headScripts = HEAD_PRESET.replace('{{ __mode__ }}', mode).replace('{{ __profile__ }}', profile).replace('{{ __version__ }}', version);
...
module.exports = WebpackPluginUmiHtml;
