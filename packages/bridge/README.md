...
// chunk-1.js
const [ sdk ] = await head.bridge.ready(['com.example.sdk@_']); // the default key _
sdk.do();

// chunk-2.js
const [ sdk ] = await head.bridge.ready(['com.example.sdk']); // the default key _
sdk.do();
```
...
// chunk-2.js
const [ sdk ] = await head.bridge.ready(['com.example.sdk@2.0.0']); // always the 1st registerd, i.e. 1.0.0 in this case
sdk.do();
```
...
```javascript
// umi.js
head.bridge.register('com.example.sdk@1.0.0', [ 'http://cdn.example.com/sdk-1.0.0.css', 'http://cdn.example.com/sdk-1.0.0.js' ], (register) => {
  const { SdkFactory } = window;
  const sdk = new SdkFactory();
  register(sdk);
});
