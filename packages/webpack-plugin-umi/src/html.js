import HtmlWebpackPlugin from 'html-webpack-plugin'; // eslint-disable-line import/no-unresolved
...
function _findBuild(js) {
  ...
  let buildEntry = '';
  js.forEach((j) => {
    const [ _0, core ] = j.match(REG_CACHED_CORE) ?? []; // eslint-disable-line no-unused-vars
    if (core) {
      buildCore = core;
    }
    const [ _1, entry ] = j.match(REG_CACHED_ENTRY) ?? []; // eslint-disable-line no-unused-vars
    if (entry) {
      buildEntry = entry;
    }
  });
...
export default WebpackPluginUmiHtml;
